...
class LLMClient:
    ...
    def create_message(self, system_prompt: str, user_prompt: str,
        ...
        # 内存缓存检查
        cache_key = f"{system_prompt[:200]}:{user_prompt[:200]}:{max_tokens}:{use_json}"
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                result = self._do_call(system_prompt, user_prompt,
                                       max_tokens, enable_caching, use_json)
    ...
    def _do_call(self, system_prompt: str, user_prompt: str,
                 max_tokens: int, enable_caching: bool, use_json: bool) -> str:
        if self.provider in ("openai", "ollama"):
